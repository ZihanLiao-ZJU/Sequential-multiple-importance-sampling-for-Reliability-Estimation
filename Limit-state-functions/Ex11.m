classdef Ex11
    % Example 11: nonlinear PF (nD)   [paper Table 4]
    %
    % Definition
    % ---------------------------------------------------------------------
    % g(u) = beta0 - sum(u)/sqrt(n) - k*(u1 - u2)^2 / 4
    % default k = -10.0;  Pf_ref = 4.75e-6   (other k values in the code)
    % Failure event:  g(u) < 0
    %
    % Configuration
    % ---------------------------------------------------------------------
    ...
    % 3) Reference failure probability (k = -10.0): Pf_ref = 4.75e-6
    %
    % Reference
    % ---------------------------------------------------------------------
    % [48] O. Ditlevsen, H.O. Madsen, Structural reliability methods, Wiley, New York, 1996.
    %
    % Syntax
    % ---------------------------------------------------------------------
    % obj = Ex11(Ndim)
