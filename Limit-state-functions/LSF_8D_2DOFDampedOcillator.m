classdef LSF_8D_2DOFDampedOcillator
    % 2-DOF damped oscillator LSF (8D)
    % Configuration
    % ---------------------------------------------------------------------
    % 1. Dimension: 8
    % 2. Pf_ref = 3.78e-7
    ...
    % Reference:
    % ---------------------------------------------------------------------
    % [1] Kiureghian, A. D., & Stefano, M. D. Efficient algorithm for second-order
    %     reliability analysis. Journal of Engineering Mechanics, 117 (1991), 2904–2923.
    %     https://doi.org/10.1061/(ASCE)0733-9399(1991)117:12(2904)
    % ---------------------------------------------------------------------
    ...
    methods
        function obj = LSF_8D_2DOFDampedOcillator
            Ndim = obj.Ndim;
            dist = [];
            sigma = zeros(Ndim,1);
            mu = zeros(Ndim,1);
