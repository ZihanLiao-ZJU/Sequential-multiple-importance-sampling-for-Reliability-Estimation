classdef Ex1
    % Example 1: piecewise-linear PF (2D)   [paper Table 1]
    %
    % Definition
    % ---------------------------------------------------------------------
    % g(u) = min( g1(u1), g2(u2) )
    ...
    % ---------------------------------------------------------------------
    % 1) Dimension: 2
    % 2) Reference distribution: independent standard normal (implemented via a Nataf object)
    % 3) Reference failure probability: Pf_ref = 3.20e-5
    %
    % Reference
    % ---------------------------------------------------------------------
    ...
    properties
        ...
        % -----------------------------------------------------------------
        % Parm   PF parameters packed as [a1;a2;a3;a4;b1;b2;b3;b4]              double [8,1]
    ...
    methods
        function L = EvlLSF(obj,theta)
            ...
            % Notes
            % -----------------------------------------------------------------
            % - The PF is constructed as the minimum of two 1D piecewise-linear
