classdef Ex8
    % Example 8: reinforced concrete beam reliability problem (7D)   [paper Table 1]
    %
    % Definition
    % ---------------------------------------------------------------------
    % g(X) = X2*X3*X4 - X5*X3^2*X4^2 / (X6*X7) - X1
    % Failure event:  g(u) < 0
    %
    % Configuration
    % ---------------------------------------------------------------------
    % 1) Dimension: 7
    % 2) Input RVs: independent with mixed Gumbel (GEV, k=0) + normal + lognormal distributions
    % 3) Reference failure probability: Pf_ref = 3.20e-3
    %
    % Reference
    % ---------------------------------------------------------------------
    % [45] J. Gong, P. Yi, N. Zhao, Non-gradient-based algorithm for structural reliability analysis, Journal of Engineering Mechanics 140 (2014) 04014029. https://doi.org/10.1061/(ASCE)EM.1943-7889.0000722.
    %
    % Syntax
    % ---------------------------------------------------------------------
    % obj = Ex8
    ...
    methods
        ...
        function Z = EvlLSF(~, X)
            ...
            % OUTPUT:
            % Z : PF values; failure corresponds to Z <= 0                  [1,   Nsam]
            % ---------------------------------------------------------------------
