classdef LSF_nD_LinearLSF1
    % Limit state function: linear hyperplane in standard normal space
    % Syntax
    % ---------------------------------------------------------------------
    % obj = LSF_nD_LinearLSF1(Ndim)
    % ---------------------------------------------------------------------
    % Configuration
    % ---------------------------------------------------------------------
    % 1. Dimension: n (user-specified)
    % 2. Pf (for hyperplane alpha^T u >= beta with ||alpha||=1):
    %    Pf = 9.87e-10; (beta=6)
    %    Pf = 2.87e-7;  (beta=5)
    %    Pf = 3.17e-5;  (beta=4)
    %    Pf = 1.35e-3;  (beta=3, default in this implementation)
    %    Pf = 2.28e-2;  (beta=2)
    % ---------------------------------------------------------------------
    % Reference:
    % ---------------------------------------------------------------------
    % [1] Dai, H., & Wang, W. Application of low-discrepancy sampling method in structural
    %     reliability analysis. Structural Safety, 31 (2009), 55–64.
    %     -- Section 4.1, Example 1
    % [3] Engelund, S., & Rackwitz, R. A benchmark study on importance sampling techniques
    ...
    methods
        function obj = LSF_nD_LinearLSF1(Ndim)
            obj.Ndim = Ndim;
...
%             title('$\it{Linear\ LSF}$','Interpreter','latex')
